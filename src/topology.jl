...
function one_dim_lattice_generator(L::Integer, lat_const::Real = 1.0)
    ...
    dist_mat = ones(L,L)*(L+1)          # Distance matrix: all entries are L+1 which is larger than the longest path (L)
...
function graphene_generator(L::Integer, C::Integer, CC_bond::Float64 = 1.0)
    if L%2 == 1
        throw(ArgumentError("L has to be an even number"))
    elseif C < 2
...
# NOTE: very naive algorithm, O(N^3)
